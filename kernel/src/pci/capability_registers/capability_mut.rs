//! The [`CapabilityEntryMut`] type for a mutable view into a PCI device's capability list

use crate::pci::{
    capability_registers::MessageSignalledInterruptsCapabilityMut, PciMappedFunction,
};

use super::MsixCapabilityMut;

/// A type of capability entry on a PCI device.
///
/// This struct represents a mutable view. If mutability is not needed, use [`CapabilityEntry`] instead.
///
/// [`CapabilityEntry`]: super::capability_const::CapabilityEntry
#[derive(Debug)]
pub enum CapabilityEntryMut<'a> {
    ...
    MessageSignalledInterrupts(MessageSignalledInterruptsCapabilityMut<'a>),
...
impl<'a> CapabilityEntryMut<'a> {
    /// # Safety
    /// * `offset` is the register (not byte) index of a capabilities structure in the configuration space of `function`
    ///
    /// # Returns
    /// The parsed entry, and the index of the next capability in the list
    pub(super) unsafe fn new(function: &mut PciMappedFunction, offset: u8) -> (Self, u8) {
        ...
        let entry = match id {
            ...
            // and the id value is 0x05 so it is an MSI capability
            0x05 => unsafe {
                Self::MessageSignalledInterrupts(MessageSignalledInterruptsCapabilityMut::new(
                    function, offset,
                ))
            },
            ...
            0x11 => unsafe { Self::MsiX(MsixCapabilityMut::new(function, offset)) },
